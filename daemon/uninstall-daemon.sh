...
# Yeni isim
PLIST_NEW="$HOME/Library/LaunchAgents/com.seyoweb.premierseyo.daemon.plist"
...
if [ "$PURGE" = "1" ]; then
  ...
  # Plugin install dizini (com.seyoweb.premierseyo_*)
  if [ -d "$PLUGIN_BASE" ]; then
    for D in "$PLUGIN_BASE"/com.seyoweb.premierseyo_*; do
